...
SELECT DEPT_TITLE,SUM(A.SALARY)
FROM EMPLOYEE A
JOIN DEPARTMENT
ON A.DEPT_CODE = DEPT_ID 
GROUP BY DEPT_TITLE     
HAVING SUM(A.SALARY) > (SELECT SUM(B.SALARY) * 0.2 FROM EMPLOYEE B);



-- 11. 나이 상 가장 막내의 사원 코드, 사원 명, 나이, 부서 명, 직급 명 조회  
SELECT EMP_ID, EMP_NAME,FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO,0,6))) / 365), DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
JOIN DEPARTMENT
ON DEPT_CODE = DEPT_ID
JOIN JOB USING(JOB_CODE)
WHERE FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO,0,6))) / 365) = (SELECT MIN(FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO,0,6))) / 365))  FROM EMPLOYEE);
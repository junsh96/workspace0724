 ...
 *  - 저장된 데이터는 브라우저를 껐다 켜도 유지가되며, 도메인별로 유지가 된다.
 *  - 최대 저장 용량은 5MB (브라우저별로 다를수 있음)
 * 
 *  localStorage.setItem(key,value); 데이터를 저장
    localStorage.getItem(key); 데이터를 호출
...
function toggleTodo(id) {
    //해당 아이디를 통해서 할일을 찾아 완료상태면 미완료로 미완료면 완료로 변경
    for (let todo of todos) {
        if(todo.id === id) {
...
//현재 필터에 따라서 할일 목록을 필터링 하여 반환하는 함수.
...
function todoItemRender(todo) {
    ...
    //새로 생성된 요소들 중에서 이벤트가 필요한 부분.
    const checkBox = todoItem.querySelector(".todo-checkbox");//todoItem내부의 체크박스.
    ...
    const deleteBtn = todoItem.querySelector(".delete-btn"); //todoItem내부의 삭제 버튼.
...
function setFilter(filter) {
    filterState = filter;//전역 상태의 필터상태를 변경.

    //모든 필터 버튼의 active 클래스 조회해서 수정.
    filterBtns.forEach(function(btn) {

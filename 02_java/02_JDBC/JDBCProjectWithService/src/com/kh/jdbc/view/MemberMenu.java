...
public class MemberMenu {
	...
	public void mainMenu() {
		while(true) {
			...
			case 5:
				...
				break;
			case 6:
				bulkINsertMembers(); //추가할 인원수 입력받아서 
				break;
			case 7:
				bulkDeleteMembers(); // ,로 아이디 구분 입력받아서.
				break;
			case 9:
	...
	public void bulkDeleteMembers() {
		System.out.println("삭제할 아이디를 ,로 구분지어 입력하세요.");
		String delUserId = sc.nextLine();
		
		mc.bulkDeleteMembers(delUserId);
	...
	public void bulkINsertMembers() {
		System.out.println("입력할 인원수를 적어주세요.");
		int cnt = sc.nextInt();
		HashMap<String, Member> map = new HashMap<>();
		for (int i=0; i<cnt; i++) {
			System.out.println("====회원 추가"+(i+1)+"====");
			...
			map.put(Integer.toString(i), m);
		}
		
		
		mc.bulkINsertMembers(map);
		
	}
	
	public void selectKeyWord() {
		System.out.println("====회원키워드검색====");

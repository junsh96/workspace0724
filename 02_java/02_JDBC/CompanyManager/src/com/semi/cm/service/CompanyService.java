package com.semi.cm.service;

import static com.semi.cm.common.JdbcConnection.*;

import java.sql.Connection;
import java.util.List;

import com.semi.cm.model.dao.CompanyDao;
import com.semi.cm.model.vo.DeptVo;
import com.semi.cm.model.vo.EmpVo;
import com.semi.cm.model.vo.JobVo;
import com.semi.cm.model.vo.MemberVo;

/**
 * 서비스
 */
public class CompanyService {

	private CompanyDao cd;
	
	
	
	public CompanyService() {
		super();
		this.cd = new CompanyDao();
	}



	/**
	 * 회원 가입(직원 추가)
	 * @param m
	 * @return
	 */
	public int signEmp(EmpVo em) {
		Connection conn = getConnection();
		
		String empDept = em.getEmpDept();
		String empJob = em.getEmpJob();
		//입력한 부서명과 직급명이 존재하는지 체크
		int deptCnt = cd.checkDept(empDept, conn);
		
		int jobCnt = cd.checkJob(empJob, conn);
		if (deptCnt == 0) {
			System.out.println("부서명을 확인해주세요.");
			return 0;
		}
		if (jobCnt == 0) {
			System.out.println("직급명을 확인해주세요.");
			return 0;
		}
		
		int result = cd.signEmp(em,conn);
		
		if (result > 0) {
			commit(conn);
			
		} else {
			rollback(conn);
		}
		
		close(conn);
		
		return result;
	}
	
	/**
	 * 로그인
	 * @param e
	 * @return
	 */
	public List<EmpVo> loginEmp(EmpVo em) {
		Connection conn = getConnection();
		List<EmpVo> result = cd.loginEmp(em,conn);
		
		close(conn);
		return result;
	}
	
	/**
	 * 회원 조회
	 * @param e
	 * @return
	 */
	public List<EmpVo> selectEmpList(EmpVo em) {
		Connection conn = getConnection();
		List<EmpVo> result = cd.selectEmpList(em,conn);
		
		
		close(conn);
		return result;
		
	}
	
	/**
	 * 회원 정보 수정
	 * @param e
	 * @return
	 */
	public int updateEmp(EmpVo em) {
		Connection conn = getConnection();
		//기존 정보 불러오기
		List<EmpVo> updateEmp = cd.selectUpdateEmp(em, conn);
		//수정 사항 값이 null 또는 공백인 경우 기존 값으로 대체 (xml에서 if test태그가 필요. nybatis에서만?)
		
		if (!(updateEmp.size() > 0)) {
			rollback(conn);
			close(conn);
			return 0;
		}
		int deptCnt  = 0; // 입력한 부서가 존재하는지 확인
		int jobCnt = 0; // 입력한 직급이 존재하는지 확인
		
		
		//입력한 부서명과 직급명이 존재하는지 체크 / 입력한 직급 및 부서가 없을 경우 기존 데이터로 덮어 씌운다.
		if (em.getEmpDept().equals("") || em.getEmpDept() == null) {
			
			em.setEmpDept(updateEmp.get(0).getEmpDept());
			deptCnt = 1;
			
		} else {
			String empDept = em.getEmpDept();
			deptCnt = cd.checkDept(empDept, conn);
			//입력한 부서와 직급의 사원수 증가			
			//기존 부서 사원수 차감

		}
		
		if (em.getEmpJob().equals("") || em.getEmpJob() == null) {
			
			em.setEmpJob(updateEmp.get(0).getEmpJob());
			jobCnt = 1;

		} else {
			String empJob = em.getEmpJob();
			jobCnt = cd.checkJob(empJob, conn);
			//입력한 부서와 직급의 사원수 증가
			//기존 직급 사원수 차감
		}
		

		if (deptCnt == 0) {
			System.out.println("부서명을 확인해주세요.");
			rollback(conn);
			close(conn);
			return 0;
		}
		if (jobCnt == 0) {
			System.out.println("직급명을 확인해주세요.");
			rollback(conn);
			close(conn);
			return 0;
		}

		

		int result = cd.updateEmp(em,conn);

		if (result > 0) {
			commit(conn);
		} else {
			rollback(conn);
		}
		
		close(conn);
		
		return result;
	}
	
	/**
	 * 회원 탈퇴
	 * @param e
	 * @return
	 */
	public int deleteEmp(EmpVo em) {
		int result =0;
		Connection conn = getConnection();
		
		
		result = cd.deleteEmp(em,conn);
		
		//트리거 필요. 부서, 직급 카운트 감소
		
		if (result > 0) {
			commit(conn);
		} else {
			result = 0;
			rollback(conn);
		}
		
		close(conn);
		return result;
	}
	
	/**
	 * 부서조회
	 * @return
	 */
	public List<DeptVo> selectDept() {
		Connection conn = getConnection();
		List<DeptVo> result = cd.selectDept(conn);
		
		close(conn);
		return result;
	}
	
	/**
	 * 직급 조회
	 * @return
	 */
	public List<JobVo> selectJob() {
		Connection conn = getConnection();
		List<JobVo> result = cd.selectJob(conn);
		
		close(conn);
		return result;
		
	}
	
	public void rollback2() {
		
		Connection conn = getConnection();
		//commit(conn);
		rollback(conn);
	}
	
}

...
<body>
    ...
    <p>
    	...
    	EL 구문을 사용하면 표현하고자하는 값을 ${name}의 형식으로 출력할수 있다. <br>
    ...
    <%--
    	JSP구성 요소
    	1. 스크립팅 원소
    		- JSP에서 직접 자바 코드를 작성할수 있는 영역
    		- 예 : 스크립틀릿(<% %>), 표현식(<%= %>),선언문(<%! %>) 
    	
    	2. 지시어
    		- JSP페이지의 메타데이터, 다른 JSP 포함, 라이브러리 등록 등을 설정
    		- 예 : <%@ page %>, <%@ include %>, <%@ taglib %>
    		
    	3. 액션태그(Action Tag)
    		- JSP에서 특정 동작을 수행하도록 지시하는 태그
    		- XML문법 기반으로 JSP기능을 확장
    		>> 표준 액션 태그 : <jsp:태그명></태그명> 접두어를 사용
    		>> 커스텀 액션 태그 : 별도 라이브러리로 등록하여 사용하는 태그(JSTL등)
     ...
     <p>
     	변수, 조건문, 반복문과 같은 로직과 관련된 문법을 태그로 제공
     </p>

...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 *  - 애플리케이션 당 1개 존재하는 객체 전역에 공유, 서버 재시작 전 까지 유지
		 *  - 멀티스레드 환경에서 공유되므로 동시성을 주의
		 *  
		 * 2) HttpSession (session scope)
		 *  - 브라우저(사용자) 당 1개 존재하는 객체(로그인상태, 사용자별 데이터 저장)
		 *  -이 영역에 데이터를 담으면 jso/servlet단에서 사용이 가능.
		 *  
		 * 3) HttpServletRequest(request scope)
		 *  - 요청마다 1개 존재하는 객체.
		 *  - 이 영역에 데이터를 담으면 request객체를 포워딩 받는 응답 jsp까지 사용이 가능하다.
		 *  
		...
		session.setAttribute("lecture", new Person("test",22,"여자"));
		
		//3) request scope
		request.setAttribute("scope","request");
		request.setAttribute("student",new Person("test22",30,"남자"));
